...
function ChatContent() {
  ...
  async function handleSend(messageText?: string) {
    ...
    setMessages((m) => [...m, { id: assistantId, role: 'assistant', text: '', time: Date.now() }])
    ...
    try {
      ...
      setMessages((m) => m.map((msg) =>
        msg.id === assistantId ? { ...msg, text: `Error: ${error.message}` } : msg
      ))
    } finally {
      setLoading(false)
    }
        ...
        {messages.length === 0 ? (
          ...
          <div className="flex-1 flex flex-col h-full">
            <div ref={messagesRef} className="flex-1 overflow-auto px-6 py-8 space-y-6 scroll-smooth">
              {messages.map((m, i) => (
                <div key={m.id ?? i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'} items-start gap-3`}> 
                  {m.role === 'user' && (
                    <>
                      ...
                      <div className="w-10 h-10 rounded-full bg-gradient-to-br from-blue-500 to-cyan-400 flex items-center justify-center text-white font-bold text-sm shadow-lg flex-shrink-0">U</div>
                    </>
                  )}
                  {m.role !== 'user' && (
                    <>
