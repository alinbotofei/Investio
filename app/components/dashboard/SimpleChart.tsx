...
export default function SimpleChart({
  ...
  if (chartData.length === 0) {
    return (
      <div className="w-full">
        ...
        <div
          className="border border-slate-700/50 rounded-lg overflow-hidden bg-slate-900 flex items-center justify-center"
  ...
  const scaleX = (x: number) =>
    PADDING +
    (x / (chartData.length - 1)) * (CHART_WIDTH - PADDING - RIGHT_PADDING);
  const scaleY = (y: number) =>
    CHART_HEIGHT -
    PADDING -
    ((y - minY) / range) * (CHART_HEIGHT - 2 * PADDING);
  ...
  const areaData = `M ${scaleX(0)} ${CHART_HEIGHT - PADDING} L ${scaleX(
    0
  )} ${scaleY(chartData[0].y)} ${pathData.substring(1)} L ${scaleX(
    chartData.length - 1
  )} ${CHART_HEIGHT - PADDING} Z`;
    ...
    <div className="w-full">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-lg font-bold text-white">
          {symbol} Price Chart
          {loading && (
            <span className="ml-2 text-xs text-slate-400 animate-pulse">
              Loading...
            </span>
          )}
        </h3>
      ...
      <div className="border border-slate-700/50 rounded-lg overflow-hidden bg-slate-900">
        ...
        >
          ...
          <text
            x={CHART_WIDTH - RIGHT_PADDING + 10}
            y={scaleY(maxY) + 4}
            fill="#94a3b8"
            fontSize="12"
