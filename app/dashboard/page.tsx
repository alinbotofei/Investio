...
function DashboardContent() {
  ...
  const [selectedStock, setSelectedStock] = useState<string>(tickerFromUrl || 'AAPL');
  ...
  useEffect(() => {
    const fetchMetrics = async () => {
      ...
      try {
        ...
        setMetrics({
          marketCap: formatNumber(metricsData.metric?.marketCapitalization || 0),
          pe: metricsData.metric?.peBasicExclExtraTTM?.toFixed(2) || 'N/A',
          high52w: metricsData.metric?.['52WeekHigh'] ? `$${metricsData.metric['52WeekHigh'].toFixed(2)}` : 'N/A',
          low52w: metricsData.metric?.['52WeekLow'] ? `$${metricsData.metric['52WeekLow'].toFixed(2)}` : 'N/A',
          volume: formatNumber(quoteData.volume || 0, ''),
          avgVolume: 'N/A',
          beta: metricsData.metric?.beta?.toFixed(2) || 'N/A',
          eps: metricsData.metric?.epsBasicExclExtraAnnual?.toFixed(2) || metricsData.metric?.epsTTM?.toFixed(2) || 'N/A'
        });
      } catch (error) {
          ...
          <div className="lg:col-span-6 space-y-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                {loadingMetrics ? (
                  Array.from({ length: 8 }).map((_, i) => (
                    <div key={i} className="p-3 bg-slate-700/50 rounded-lg animate-pulse">
                  ...
                  [
                    ...
                    <div key={metric.label} className="p-3 bg-slate-700/50 rounded-lg transition-all hover:bg-slate-700">
            ...
            <div className="bg-slate-800/90 border border-slate-700/50 rounded-lg p-6">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h3 className="text-lg font-bold text-white">Ask Investio about {selectedStock}</h3>
              </div>
